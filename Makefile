...
CFLAGS=-c -g -O3 -fPIC -Wall -Werror -Wsign-compare -Isrc -Ihtml -Ipdf -Ipdf/2.3.0RC2/include -Lpdf/2.3.0RC2/lib
LDFLAGS=-g -O3 -Wall -Werror -lhpdf
CC=gcc
...
all:		libsundown.so sundown smartypants html_blocks markdownpdf
...
markdownpdf:	examples/markdownpdf.o $(SUNDOWN_SRC)
	$(CC) $(LDFLAGS) $^ -o $@
...
# housekeeping
clean:
	rm -f src/*.o html/*.o examples/*.o pdf/*.o
	rm -f libsundown.so libsundown.so.1 sundown smartypants
	rm -f sundown.exe smartypants.exe markdownpdf
	rm -rf $(DEPDIR)
...
%.o:	src/%.c examples/%.c html/%.c pdf/%.c
	@mkdir -p $(DEPDIR)
	@$(CC) -MM $< > $(DEPDIR)/$*.d
	$(CC) $(CFLAGS) -o $@ $<
